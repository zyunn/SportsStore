...
class ViewController: UIViewController, UITableViewDataSource {
    ...
    var productDataStore=ProductDataStore()
...
//            category:"Chess", price:1200.0, stockLevel:4)];
    ...
    func tableView(_ tableView: UITableView,
        cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let product = productDataStore.products[(indexPath as NSIndexPath).row];
            let cell = tableView.dequeueReusableCell(withIdentifier: "ProductCell")
                as! ProductTableCell;
            cell.product = productDataStore.products[(indexPath as NSIndexPath).row];
                ...
                if let cell = currentCell as? ProductTableCell {
                    if let product = cell.product{
                        if let stepper = sender as? UIStepper {
                            ...
                            if let newValue = textfield.text?.hashValue {
                                product.stockLevel = newValue;
                            }
                        }
                        cell.stockStepper.value = Double(product.stockLevel);
                        cell.stockField.text = String(product.stockLevel);
                        productLogger.logItems(item: product)
                    }
                    break;
                }
            }
    ...
    func displayStockTotal(){
        let finalTotals:(Int,Double)=productDataStore.products.reduce((0,0.0)) { (totals, product) -> (Int,Double) in return(
